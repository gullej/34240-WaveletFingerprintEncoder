function [psnr, len, bpp, mse, per, rate] = evaluateCompression(img, bpp)
...
len = (size(encodedseq,2)+8*size(byteneg,2)+8*size(bytepos,2)+8*size(byterun,2)+16*size(doubleneg,2)+16*size(doublepos,2)+16*size(doublerun,2));
bpp = len/(640*480);
per = len/(640*480*8);
rate = 1/per;
